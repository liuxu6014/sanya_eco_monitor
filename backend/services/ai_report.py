...
- 监测数据只要在摘要中出现，就必须纳入分析，禁止只分析其中一部分
- 必须开展“多源联合分析”，把虫情、孢子、降雨、径流、水质放在同一生态过程链条中综合讨论
...
- “监测体系科学性说明”和“基准期口径”并入第一章；“水土流失监测型估算”和“气象支撑信息”并入第三章；“农业面源污染削减率”和相关口径说明并入第四章；“病虫风险链条”并入第二章、第六章和第七章
...
【全量分析强制要求】
1. 你必须逐项检查并使用以下模块的全部可用数据：虫情、孢子、降雨、地表径流、水质。
2. 只要某模块监测记录数大于 0，就必须在正文中明确分析该模块，不能省略。
3. 必须把不同模块之间的关系说清楚，例如：降雨与径流、水文与水质、孢子与湿润环境、虫情与生物多样性。
4. 对地表径流必须结合各监测点分别分析，不能只写总体平均值。
5. 如果某模块监测值较低或为 0，应解释其生态含义、监测背景和可能原因，而不是直接跳过。
6. 结论必须来自摘要中的真实数据，严禁编造不存在的设备、指标、峰值或趋势。
7. 如果摘要中提供了 guideline_metrics 或 weather_support，必须纳入正文，重点说明水土流失监测型估算、面源污染削减率、病虫风险链条和气象支撑信息。
...
def format_summary_for_prompt(summary: dict) -> str:
    ...
    lines.append("孢子监测：")
    _append_fact(lines, "监测记录数", sp.get("records_count", 0), " 条")
    _append_fact(lines, "周期内有效捕获孢子", sp.get("total_count", 0), " 个")
    _append_fact(lines, "实拍图片数量", len(sp.get("capture_images") or []), " 张")
    if sp.get("daily"):
        lines.append("  - 孢子日尺度序列：")
        for item in sp["daily"]:
            lines.append(f"    * {item.get('date', '—')}：捕获 {item.get('count', 0)} 个")
    ...
    if pest_guideline.get("available"):
        lines.append("需融入第二章、第六章和第七章的病虫风险链条：")
        ...
        _append_fact(
            lines,
            "孢子峰值",
            f"{pest_guideline.get('spore_peak', {}).get('date', '—')} / "
            f"{pest_guideline.get('spore_peak', {}).get('count', 0)} 个",
        )
        _append_fact(lines, "建议动作", pest_guideline.get("suggestion"))
        _append_fact(lines, "闭环说明", pest_guideline.get("chain_text"))

    if warning_analysis:
        warning_summary = warning_analysis.get("summary") or {}
        lines.append("需融入第二章和第三章的分级预警判定：")
        _append_fact(lines, "预警总览", warning_summary.get("headline"))
