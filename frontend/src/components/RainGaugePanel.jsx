...
export default function RainGaugePanel({ rainData, deviceMeta = [] }) {
  ...
  const devices = Array.isArray(deviceMeta) ? deviceMeta : [];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', height: '100%', padding: '4px 0' }}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '8px', overflowY: 'auto' }} className="no-scrollbar">
        {devices.map((device) => {
          ...
          const name = device.panel_name || device.name || `${code} 雨量站`;
          const realtimeRainfall = typeof d?.realtime_rainfall === 'number' ? d.realtime_rainfall : null;
            ...
            <div key={code} style={{
              ...
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: '18px', color: d ? '#38bdf8' : '#475569', fontWeight: 'bold' }}>
                  {realtimeRainfall != null ? realtimeRainfall.toFixed(1) : '--'}
